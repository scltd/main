...
public class BookBorg implements EventNotifierInterface, OrderEntryInterface {
	...
	public void handleOrder(Order order) {
		...
		try {
			...
			log.trace("Order received for {}-{} contract.", c.getUuid(),
					c.getSymbol_name());
			...
			if (order.getQty() == 0) {
				...
				if (ord == null) {
					...
					log.trace(
							"We have no knowledge of any orders for contract: {}. doing insert.",
							c.getUuid());
					ord = new OrderWrapper(order);
					ord.set_externalContract(c);
					ord.set_id(_order_id);
					_externalConnection
							.placeOrder(ord.get_id(),
									ord.get_externalContract(),
					...
					if (ord.isAcked()) {
						...
						log.warn(
								"Order {} hasn't been acked. Ignoring this update.",
								ord.get_id());
				}
			}

		} catch (ContractNotFoundException e) {
			log.warn("Order received for unknown contract {}. Discarding it.",
					order.getContract());
	...
	public void handleOrderAck(int orderId) {
		...
		if (ord == null)
			log.warn("Somehow we've got confirmation for unknown order id: {}",
					orderId);
		else {
	...
	public void handleOrderFill(int orderId, double avgFillPrice, int fillQty) {
		log.trace("Handle OrderFill: {} price: {}, shares: {}", orderId,
				avgFillPrice, fillQty);
