...
public class Types {
  ...
  public static enum AlgoParam {
    startTime, endTime, allowPastEndTime, maxPctVol, pctVol, strategyType, noTakeLiq, riskAversion, forceCompletion, displaySize, getDone, noTradeAhead, useOddLots, componentSize, timeBetweenOrders, randomizeTime20, randomizeSize55, giveUp, catchUp, waitForFill
  }

  public static enum AlgoStrategy implements IApiEnum {
    None(), Vwap(startTime, endTime, maxPctVol, noTakeLiq, getDone, noTradeAhead, useOddLots), Twap(
        startTime, endTime, allowPastEndTime, strategyType), ArrivalPx(startTime, endTime,
        allowPastEndTime, maxPctVol, riskAversion, forceCompletion), DarkIce(startTime, endTime,
        allowPastEndTime, displaySize), PctVol(startTime, endTime, pctVol, noTakeLiq), AD(
        startTime, endTime, componentSize, timeBetweenOrders, randomizeTime20, randomizeSize55,
  ...
  public static enum Rule80A implements IApiEnum {
    None(""), IndivArb("J"), IndivBigNonArb("K"), IndivSmallNonArb("I"), INST_ARB("U"), InstBigNonArb(
        "Y"), InstSmallNonArb("A");
  ...
  public static <T extends Enum<T>> T getEnum(int ordinal, T[] values) {
    ...
    String str =
        String.format("Error: %s is not a valid value for enum %s", ordinal, values[0].getClass()
            .getName());
