...
public class EClientSocket {
  ...
  // 27 = can receive delta neutral order type and delta neutral aux price in place order version
  // 20: API 8.85
  ...
  // 52 = can receive deltaNeutralConId, deltaNeutralSettlingFirm, deltaNeutralClearingAccount and
  // deltaNeutralClearingIntent in openOrder
  // 53 = can receive orderRef in execution
  // 54 = can receive scale order fields (PriceAdjustValue, PriceAdjustInterval, ProfitOffset,
  // AutoReset,
  ...
  // 60 = can receive deltaNeutralOpenClose, deltaNeutralShortSale, deltaNeutralShortSaleSlot and
  // deltaNeutralDesignatedLocation in openOrder
  ...
  // 63 = can receive verifyMessageAPI, verifyCompleted, displayGroupList and displayGroupUpdated
  // messages
  ...
  protected synchronized void startAPI() {
    ...
    try {
      ...
      error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_STARTAPI, "" + e);
      close();
    }
  }
    ...
    try {
      ...
      if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
        ...
        int scannerSubscriptionOptionsCount =
            scannerSubscriptionOptions == null ? 0 : scannerSubscriptionOptions.size();
  ...
  public synchronized void reqMktData(int tickerId, Contract contract, String genericTickList,
      boolean snapshot, List<TagValue> mktDataOptions) {
    if (!m_connected) {
    ...
    try {
      ...
      if (m_serverVersion >= 31) {
        /*
         * Note: Even though SHORTABLE tick type supported only starting server version 33 it would
         * be relatively expensive to expose this restriction here.
         * 
  ...
  /**
   * Note that formatData parameter affects intra-day bars only; 1-day bars always return with date
   * in YYYYMMDD format.
   */
  public synchronized void reqHistoricalData(int tickerId, Contract contract, String endDateTime,
  ...
  public synchronized void reqRealTimeBars(int tickerId, Contract contract, int barSize,
      String whatToShow, boolean useRTH, Vector<TagValue> realTimeBarsOptions) {
    // not connected?
    if (!m_connected) {
      notConnected();
      return;
  ...
  public synchronized void reqMktDepth(int tickerId, Contract contract, int numRows,
      Vector<TagValue> mktDepthOptions) {
    // not connected?
    if (!m_connected) {
      notConnected();
      return;
  ...
  public synchronized void exerciseOptions(int tickerId, Contract contract, int exerciseAction,
  ...
  public synchronized void placeOrder(int id, Contract contract, Order order) {
    ...
    if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
      if (order.m_deltaNeutralConId > 0 || !IsEmpty(order.m_deltaNeutralSettlingFirm)
    ...
    if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE) {
      if (!IsEmpty(order.m_deltaNeutralOpenClose) || order.m_deltaNeutralShortSale
    ...
    if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3) {
      if (order.m_scalePriceIncrement > 0 && order.m_scalePriceIncrement != Double.MAX_VALUE) {
        if (order.m_scalePriceAdjustValue != Double.MAX_VALUE
            ...
            || order.m_scaleProfitOffset != Double.MAX_VALUE || order.m_scaleAutoReset
    ...
    if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE
        && BAG_SEC_TYPE.equalsIgnoreCase(contract.m_secType)) {
    ...
    if (m_serverVersion < MIN_SERVER_VER_SCALE_TABLE) {
      if (!IsEmpty(order.m_scaleTable) || !IsEmpty(order.m_activeStartTime)
          || !IsEmpty(order.m_activeStopTime)) {
        error(id, EClientErrors.UPDATE_TWS,
    ...
    try {
      ...
      if (m_serverVersion >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS
          && BAG_SEC_TYPE.equalsIgnoreCase(contract.m_secType)) {
        java.util.Vector smartComboRoutingParams = order.m_smartComboRoutingParams;
        int smartComboRoutingParamsCount =
            smartComboRoutingParams == null ? 0 : smartComboRoutingParams.size();
      ...
      if (m_serverVersion >= 19) {
        ...
        double lower =
            (m_serverVersion == 26 && order.m_orderType.equals("VOL")) ? Double.MAX_VALUE
      ...
      if (m_serverVersion >= 26) { // Volatility orders
        ...
        if (m_serverVersion == 26) {
          // Volatility orders had specific watermark price attribs in server version 26
          double lower =
              order.m_orderType.equals("VOL") ? order.m_stockRangeLower : Double.MAX_VALUE;
          double upper =
              order.m_orderType.equals("VOL") ? order.m_stockRangeUpper : Double.MAX_VALUE;
          sendMax(lower);
          sendMax(upper);
        }
      ...
      if (m_serverVersion >= MIN_SERVER_VER_SCALE_ORDERS3 && order.m_scalePriceIncrement > 0.0
          && order.m_scalePriceIncrement != Double.MAX_VALUE) {
  ...
  public synchronized void calculateOptionPrice(int reqId, Contract contract, double volatility,
      double underPrice) {
