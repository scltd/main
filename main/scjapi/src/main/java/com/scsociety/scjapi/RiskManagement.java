...
public class RiskManagement {
  ...
  public RiskManagement(Properties config) {
    ...
    accountsInterface = new AccountsImpl<AccountModel>(config, AccountModel.class);
  ...
  public RiskModel loadAccount(String name) {
    ...
    while (ait.hasNext()) {
      ...
      Contract contract = contractsInterface.getContractById(am.getContract());
      if (contract == null) {
        log.error("Contract id: {} not found. Skipping it.", am.getContract());
        continue;
      }
  ...
  public Boolean insertPosition(Integer client_id, String exchange_symbol_id, Integer size,
      Double price) {
    if (_risks == null) {
    ...
    if (c == null) {
      log.error("Cannot find symbol mapped to exchange_id: {}", exchange_symbol_id);
      return false;
    }
    if (!tradesInterface.insertTrade(_risks.get_name(), c.getUuid(), "SCSTRD01", size, price,
        "Manualy executed", 0, true)) {
    ...
    if (!accountsInterface.updateAccount(am.getUuid(), am.getToday(), am.getEquity())) {
