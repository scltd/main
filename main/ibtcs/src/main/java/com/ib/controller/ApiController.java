...
public class ApiController implements EWrapper {
  ...
  private int m_reqId; // used for all requests except orders; designed not to conflict with
                       // m_orderId
  ...
  private final HashMap<Integer, IInternalHandler> m_contractDetailsMap =
      new HashMap<Integer, IInternalHandler>();
  ...
  private final HashMap<Integer, ITopMktDataHandler> m_topMktDataMap =
      new HashMap<Integer, ITopMktDataHandler>();
  private final HashMap<Integer, IDeepMktDataHandler> m_deepMktDataMap =
      new HashMap<Integer, IDeepMktDataHandler>();
  private final HashMap<Integer, IScannerHandler> m_scannerMap =
      new HashMap<Integer, IScannerHandler>();
  private final HashMap<Integer, IRealTimeBarHandler> m_realTimeBarMap =
      new HashMap<Integer, IRealTimeBarHandler>();
  private final HashMap<Integer, IHistoricalDataHandler> m_historicalDataMap =
      new HashMap<Integer, IHistoricalDataHandler>();
  private final HashMap<Integer, IFundamentalsHandler> m_fundMap =
      new HashMap<Integer, IFundamentalsHandler>();
  private final HashMap<Integer, IOrderHandler> m_orderHandlers =
      new HashMap<Integer, IOrderHandler>();
  private final HashMap<Integer, IAccountSummaryHandler> m_acctSummaryHandlers =
      new HashMap<Integer, IAccountSummaryHandler>();
  private final HashMap<Integer, IMarketValueSummaryHandler> m_mktValSummaryHandlers =
      new HashMap<Integer, IMarketValueSummaryHandler>();
  private final ConcurrentHashSet<IPositionHandler> m_positionHandlers =
      new ConcurrentHashSet<IPositionHandler>();
  private final ConcurrentHashSet<IAccountHandler> m_accountHandlers =
      new ConcurrentHashSet<IAccountHandler>();
  private final ConcurrentHashSet<ILiveOrderHandler> m_liveOrderHandlers =
      new ConcurrentHashSet<ILiveOrderHandler>();
  ...
  public void updatePortfolio(Contract contractIn, int positionIn, double marketPrice,
    ...
    Position position =
        new Position(contract, account, positionIn, marketPrice, marketValue, averageCost,
  ...
  public void reqAccountSummary(String group, AccountSummaryTag[] tags,
      IAccountSummaryHandler handler) {
  ...
  public void position(String account, Contract contractIn, int pos, double avgCost) {
    NewContract contract = new NewContract(contractIn);
  ...
  public void reqContractDetails(NewContract contract, final IContractDetailsHandler processor) {
    ...
    internalReqContractDetails(contract, new IInternalHandler() {
      public void contractDetails(NewContractDetails data) {
        list.add(data);
      }
  ...
  public interface IOptHandler extends ITopMktDataHandler {
    void tickOptionComputation(NewTickType tickType, double impliedVol, double delta,
  ...
  public void reqTopMktData(NewContract contract, String genericTickList, boolean snapshot,
  ...
  public void reqOptionMktData(NewContract contract, String genericTickList, boolean snapshot,
      IOptHandler handler) {
    int reqId = m_reqId++;
    m_topMktDataMap.put(reqId, handler);
  ...
  public void reqEfpMktData(NewContract contract, String genericTickList, boolean snapshot,
      IEfpHandler handler) {
    int reqId = m_reqId++;
    m_topMktDataMap.put(reqId, handler);
  ...
  public void tickEFP(int reqId, int tickType, double basisPoints, String formattedBasisPoints,
      double impliedFuture, int holdDays, String futureExpiry, double dividendImpact,
      double dividendsToExpiry) {
    IEfpHandler handler = m_efpMap.get(reqId);
    if (handler != null) {
      handler.tickEFP(tickType, basisPoints, formattedBasisPoints, impliedFuture, holdDays,
  ...
  public void updateMktDepth(int reqId, int position, int operation, int side, double price,
      int size) {
    IDeepMktDataHandler handler = m_deepMktDataMap.get(reqId);
    if (handler != null) {
      handler.updateMktDepth(position, null, DeepType.get(operation), DeepSide.get(side), price,
          size);
    }
    recEOM();
  }
  ...
  public void updateMktDepthL2(int reqId, int position, String marketMaker, int operation,
    ...
    IDeepMktDataHandler handler = m_deepMktDataMap.get(reqId);
    if (handler != null) {
      handler.updateMktDepth(position, marketMaker, DeepType.get(operation), DeepSide.get(side),
          price, size);
    }
    recEOM();
  }
  ...
  // ---------------------------------------- Option computations
  // ----------------------------------------
  public void reqOptionVolatility(NewContract c, double optPrice, double underPrice,
      IOptHandler handler) {
    int reqId = m_reqId++;
    m_optionCompMap.put(reqId, handler);
  ...
  public void tickOptionComputation(int reqId, int tickType, double impliedVol, double delta,
      double optPrice, double pvDividend, double gamma, double vega, double theta, double undPrice) {
    IOptHandler handler = m_optionCompMap.get(reqId);
    if (handler != null) {
      handler.tickOptionComputation(NewTickType.get(tickType), impliedVol, delta, optPrice,
          pvDividend, gamma, vega, theta, undPrice);
    } else {
      System.out.println(String.format("not handled %s %s %s %s %s %s %s %s %s", tickType,
  ...
  public interface IOrderHandler {
    ...
    void orderStatus(OrderStatus status, int filled, int remaining, double avgFillPrice,
  ...
  public void exerciseOption(String account, NewContract contract, ExerciseType type, int quantity,
      boolean override) {
    m_client.exerciseOptions(m_reqId++, contract.getContract(), type.ordinal(), quantity, account,
        override ? 1 : 0);
    sendEOM();
  }
  ...
  public interface ILiveOrderHandler {
    ...
    void orderStatus(int orderId, OrderStatus status, int filled, int remaining,
  ...
  public void orderStatus(int orderId, String status, int filled, int remaining,
    ...
    IOrderHandler handler = m_orderHandlers.get(orderId);
    if (handler != null) {
      handler.orderStatus(OrderStatus.valueOf(status), filled, remaining, avgFillPrice, permId,
          parentId, lastFillPrice, clientId, whyHeld);
    }

    for (ILiveOrderHandler liveOrderHandler : m_liveOrderHandlers) {
      liveOrderHandler.orderStatus(orderId, OrderStatus.valueOf(status), filled, remaining,
   ...
   */
  public void reqHistoricalData(NewContract contract, String endDateTime, int duration,
    ...
    m_client.reqHistoricalData(reqId, contract.getContract(), endDateTime, durationStr,
  ...
  public void reqRealTimeBars(NewContract contract, WhatToShow whatToShow, boolean rthOnly,
  ...
  public void reqFundamentals(NewContract contract, FundamentalType reportType,
